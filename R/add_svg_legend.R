#'@title add a custom svg legend to a leaflet map
#'@description this function is in development
#'@param parent_node an XMLInternalNode
#'@param width width of legend in px
#'@param height height of legend in px
#'@param cols colors of legend
...
add_svg_legend <- function(parent_node, width = 300, height = 100, cols, names, title){
  ...
  m_spc <- 15 # space between bars and margins
  v_spc <- 45 # space between bottom of div and base of text
  b_hgt <- 14 # bar height
  n_bars <- length(cols)
  ...
  addChildren(svg, newXMLNode('style',
                              sprintf('.my-legend{position: fixed;top: 0px;left: 30px;width: %spx;height: %spx;z-index:99;}', 
                                      width, height)))
  addChildren(svg, newXMLNode('text', title, attrs = c(x=width/2, y=height-(v_spc+20+b_hgt),'text-anchor'="middle", 'font-size'=16)))
  
  for (i in 1:length(cols)){
    x_lft = m_spc+((i-1)*bar_w+(i-1)*b_spc)
    x_cnt = x_lft+bar_w/2
    addChildren(svg, newXMLNode('rect', attrs = c('x'=x_lft, 'y'=height-(v_spc+12+b_hgt), 'rx' = 2, 'ry'=2, 
                                                      width=bar_w, height=b_hgt, style=sprintf('fill:%s;stroke:%s;opacity:0.8',cols[i],cols[i]))))  
    addChildren(svg, newXMLNode('text', names[i], attrs = c(x=x_cnt, y=height-v_spc,'text-anchor'="middle")))
  }  

  
  addChildren(leg_div, svg)
  
  parent_node <- addChildren(parent_node, leg_div)
  return(parent_node )
}

add_usgs <- function(parent_id){
    t_x = width-125#-81
    t_y = height-38#-28
    scale = 0.25
    fill = '#505050'
